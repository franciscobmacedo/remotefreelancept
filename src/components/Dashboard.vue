<template>
  <div class="grid grid-cols-1 md:grid-cols-2 md:space-x-10 mt-5 md:mt-5 mb-20">
    <div>
      <!-- left side -->
      <div class="flex items-baseline justify-center md:justify-start flex-wrap">
        <p class="text-sm text-neutral-600 mr-2 w-12/12">Show income per</p>
        <div class="flex justify-center items-center">
          <button v-for="frequencyChoice in Object.keys(FrequencyChoices)" :key="frequencyChoice" class="
              w-fit
              text-gray-700
              block
              px-4
            ...
            }" @click="setFrequency(frequencyChoice)" data-cy="frequency-button">
            {{ frequencyChoice }}
          </button>
        </div>
      </div>
      <div class="flex ml-3 md:ml-0 justify-start items-center mt-2 space-x-4">
        ...
        <AdjustCounter :value="store.nrMonthsDisplay" @update:value="store.setNrMonthsDisplay" :min="1" unit="months"
      ...
      <div class="flex ml-3 md:ml-0 justify-start items-center mt-2 space-x-4">
        ...
        <AdjustCounter v-model:value="ssDiscountPosition" :min="0" :max="ssDiscountChoices.length - 1"
          data-cy="ss-discount">
          {{ ssDiscountDisplay }}
        </AdjustCounter>
        <InfoButton
      ...
      <div class="flex ml-3 md:ml-0 justify-start items-center mt-2 space-x-4">
        ...
        <div class="w-16">
          <DropDown :choices="SUPPORTED_TAX_RANK_YEARS" @change="changeCurrentTaxRankYear"
            :value="getCurrentTaxRankYear.toString()" data-cy="tax-rank-years-dropdown" />
        </div>
        <InfoButton>
          <p class="text-sm w-64 text-center">
      ...
      <div class="
          ...
          md:space-x-4
        ">
        <SwitchButton label="Are you within the first 12 months of starting your activity?"
          :model-value="store.ssFirstYear" @update:model-value="store.setSsFirstYear" data-cy="ss-first-year" />
        <InfoButton
      ...
      <div class="
          ...
          md:space-x-4
        ">
        <SwitchButton label="Are you in your first fiscal year of activity?" :model-value="store.firstYear"
          @update:model-value="setFirstYear" :key="firstYearKey" data-cy="first-year" />
        <InfoButton
      ...
      <div class="
          ...
          md:space-x-4
        ">
        <SwitchButton label="Are you in your second fiscal year of activity?" :model-value="secondYear"
          @update:model-value="setSecondYear" :key="secondYearKey" data-cy="second-year" />
        <InfoButton
          link="https://www.cgd.pt/Site/Saldo-Positivo/leis-e-impostos/Pages/irs-trabalhadores-independentes.aspx">
          <p class="text-sm w-64 text-center">
      ...
      <div class="
          ...
          md:space-x-4
        ">
        <SwitchButton label="Are you eligible to be in the NHR/RNH?" :model-value="store.rnh"
          @update:model-value="store.setRnh" data-cy="rnh" />
        <InfoButton
      ...
      <div v-if="expensesNeeded > 0" class="flex ml-3 md:ml-0 justify-start items-center mt-6 space-x-4">
        <div>
          <p class="text-sm w-fit">
            How much can you justify as professional related expenses?
          </p>
          <span class="text-xs text-neutral-500">the maximum required is
            <span class="font-semibold">{{ asCurrency(expensesNeeded) }}</span>
            <span class="text-xs text-neutral-500">/year</span>
          </span>
        </div>
        <AdjustCounter :value="store.expenses" @update:value="store.setExpenses" :min="0" :max="grossIncome.year"
          :step="100" unit="€" :width="14" data-cy="expenses">
        </AdjustCounter>
        <InfoButton
      ...
      <div class="flex gap-x-2">
        ...
        <div class="
          ...
          <div class="
              ...
              tracking-wide
            ">
            <small>{{ irsDisplay }}<span class="text-red-400"> IRS</span></small>
